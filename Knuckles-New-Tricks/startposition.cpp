...
void Init_StartEndPos() {
	...
	WriteData((StartPosition**)0x43d947, KnuxStartArray);	
	WriteData((StartPosition**)0x43d94e, KnuxStartArray);

	WriteData((StartPosition**)0x43df82, CharacterEndArray); //Change Knux End Array with a more complete one
	WriteData((LevelEndPosition**)0x43ddf4, CharacterEndArrayM2M3); //Same with M2 and M3

	if (!isSA2Miles()) {
		WriteData((StartPosition**)0x43df89, CharacterEndArray); //Change Rouge End Array with a more complete one, Better Miles already do it so we don't have to if the mod is loaded.
